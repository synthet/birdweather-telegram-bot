# Documentation

Specs and operational docs for **birdweather-telegram-bot**, synthesized from implementation work and Cursor agent sessions (May 2026). See [HISTORY.md](./HISTORY.md) for a session log.

| Document | Purpose |
|----------|---------|
| [SPEC.md](./SPEC.md) | Product behavior: commands, auth, notifications, data |
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Modules, APIs, data flow, database |
| [OPERATIONS.md](./OPERATIONS.md) | Env vars, Docker, MCP, BotFather, security |
| [ROADMAP.md](./ROADMAP.md) | Planned integrations and deferred features |
| [HISTORY.md](./HISTORY.md) | Chronological log of major chat-driven changes |

For day-to-day setup, start with the root [README.md](../README.md), then use this folder for design detail.
